...
interface MultiStepFormBuilderProps<T> {
    steps: any[]
    initialData: T
}

function MultiStepFormBuilder<T>({ steps: passedSteps, initialData }: MultiStepFormBuilderProps<T>) {
    const { steps, currentStepIndex, data, isLastStep, prev, next } = useMultiStepForm(passedSteps, initialData)
    ...
    return (
        <section className='relative inset-0 flex flex-col justify-between p-24'>
            ...
            {/* Form */}
            <form className='mt-12 py-12' onSubmit={onSubmit}>
                {step}
            </form>
...
export function useMultiStepForm<T>(steps: any[], initialData: T) {
